...
export default function CoffeeManagment({setsee2, see2, coffeprod, setCoffeprod}) {
  ...
  useEffect(() => {
    ...
    try {
      const res = await fetch('https://distributer-website.vercel.app/local/products');
...
const updateCard = async (index) => {
  ...
  try {
    const res = await fetch(`https://distributer-website.vercel.app//local/products/${product._id}`, {
      method: "PUT",
      headers: {
...
const deleteCard =async (index,e)=>{
  ...
  try{
    const del = await fetch(`https://distributer-website.vercel.app//local/products/${todelete._id}`,{
      method: "DELETE"
    })
